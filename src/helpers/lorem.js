...
const text3 = 'At vero eos et accusamus et iusto odio dignissimos ducimus qui blanditiis praesentium voluptatum deleniti atque corrupti quos dolores et quas molestias excepturi sint occaecati cupiditate non provident, similique sunt in culpa qui officia deserunt mollitia animi, id est laborum et dolorum fuga. Et harum quidem rerum facilis est et expedita distinctio. Nam libero tempore, cum soluta nobis est eligendi optio cumque nihil impedit quo minus id quod maxime placeat facere possimus, omnis voluptas assumenda est, omnis dolor repellendus. Temporibus autem quibusdam et aut officiis debitis aut rerum necessitatibus saepe eveniet ut et voluptates repudiandae sint et molestiae non recusandae. Itaque earum rerum hic tenetur a sapiente delectus, ut aut reiciendis voluptatibus maiores alias consequatur aut perferendis doloribus asperiores repellat.';
const combinedTexts = [text1, text2, text3].join(' ').split(' ');

function* loremGen () {
    while(true) {
        const text = [...combinedTexts];
        let i;
        while(text.length) {
            i = yield text.splice(0, i);
        }  
    }
}
const q = loremGen();
q.next(); // progress to first yield

export default (jj = 70) => {
    const j = Math.min(combinedTexts.length, jj);
    let text = q.next(j).value;
    while(text.length < j) {
        text = q.next(j).value
    } 
    return text.join(' ');
